...
| `DATABASE_URL`                  | Migrations only, run via `psql`/your own tooling — not read by the app itself | Supabase's **pooled** connection string (port 6543) works for most cases. Supabase's _direct_ connection (port 5432) is IPv6-only; if your network/host is IPv4-only, use the pooler connection instead (`aws-<region>.pooler.supabase.com:6543`) — this project hit exactly that during setup. |
