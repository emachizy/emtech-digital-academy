...
- **`lesson_progress`** — one row per (profile, lesson), `status: not_started | in_progress | completed`. This is the _only_ source of truth for a student's progress; percentages/completion counts are always computed from it at query time, never stored redundantly.
